...
from query.address import get_address_details
import warnings
...
def Generator(addresses, filename, offset):
    ...
    filepath = './datasets_generated/address_info' + str(filename) + '.csv'
    transactionFilepath = './datasets_generated/transaction_info' + \
        "transaction_" + str(filename) + '.csv'
    errFilepath = './datasets_generated/skipped_address_info/' + \
    ...
    for count, address in enumerate(addresses):
        ...
        if count % 50 == 0:
            ...
            time.sleep(duration)
            print(
                f"{bcolors.WARNING}Ended Sleep{bcolors.ENDC}")
